...
const MawCoinMultiSigFactory = artifacts.require('MawCoinMultiSigWalletWithMint.sol');
...
contract("MawCoinMultiSigWalletWithMint", (accounts) => {
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestMintInMaw(user1, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestMintInMaw(user1, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Add and remove a minter", async () => {
      ...
      assertNotRevert("user1 can mint", () => coinInstance.mint(user1, OneMawCoinInMinunit, {from: user1}));
      let afterBalance = await coinInstance.balanceOf(user1);

      // balance difference should be same as the minted amount
      assert.equal(afterBalance.sub(prevBalance).toString(), OneMawCoinInMinunit.toString());
      ...
      assertRevert("user1 cannot mint after renounce", () => coinInstance.mint(user1, OneMawCoinInMinunit, {from: user1}));
  ...
  it("test mint using multisig wallet", async () => {
        ...
        const thousandMaw = BIG(1000)
        const zero = BIG(0)

        const MSG1 = "MINT 1000 Maw to user1";
        const trSubmit = await walletInstance.requestMintInMaw(user1, thousandMaw, zero, MSG1, {from:vault});
        ...
        assert.equal(afterBalance.toString(), thousandMawInMinunit.toString());
