...
const MawCoin = artifacts.require('MawCoin.sol');
const MawCoinMultiSigFactory = artifacts.require('MawCoinMultiSigWallet.sol');
const web3 = MawCoin.web3;
const eth = web3.eth;
...
const deployMultiSigWallet = (owners, requiredConfirm, coin) => {
  return MawCoinMultiSigFactory.new(owners, requiredConfirm, coin, "vault");
};

const deployMawCoin = (owner,admin,vault) => {
  return MawCoin.new("Maw", "MawCoin", owner, admin, vault);
};
...
contract("MawCoinMultiSigWallet", (accounts) => {
  const BIG = (v) => new theBN.BN(v)
  let coinInstance;
  let walletInstance;
  const requiredConfirmations = 2;

  let OneMawCoinInMinunit;
  ...
  beforeEach(async () => {
    coinInstance = await deployMawCoin(owner, admin, vault);
    assert.ok(coinInstance);

    NoOfTokensInMinunit = await coinInstance.totalSupply();
    OneMawCoinInMinunit = await coinInstance.getOneMawCoin();
  ...
  it("multisig wallet basic functions", async () => {
    assert.equal( await walletInstance.mawCoin(), coinInstance.address );
  ...
  it("only signers can request TX", async () => {
    // user1 cannot
    assertRevert("1-1",() => walletInstance.requestTransferInMaw(user1, 1, 0, "test 1", {from: user1}));
    ...
    // owner cannot
    assertRevert("2-1",() => walletInstance.requestTransferInMaw(user1, 1, 0, "test 2-1", {from: owner}));
    ...
    // admin(as signer) can
    assertNotRevert("3-1",() => walletInstance.requestTransferInMaw(user1, 1, 0, "test 3-1", {from: admin}));
    ...
    // vault(as signer) can
    assertNotRevert("4-1",() => walletInstance.requestTransferInMaw(user1, 1, 0, "test 4-1", {from: vault}));
    ...
    assertNotRevert("5-1",() => walletInstance.requestTransferInMaw(user1, 1, 0, "test 5-1", {from: signer}));
  ...
  it("signers only methods", async () => {
    // request a TX
    walletInstance.requestTransferInMaw(user1, 1, 0, "test 1", {from: signer});
  ...
  it("test transfer using multisig wallet", async () => {
    ...
    const thousandMawInMinunit = OneMawCoinInMinunit.mul(BIG(1000))
    coinInstance.transfer(walletInstance.address, thousandMawInMinunit, {from:vault});
    assert.equal(await balanceOf(walletInstance.address), thousandMawInMinunit.toString());

    // Add mawcoin transfer transaction
    // example ABI generation:
    // const transferData = coinInstance.contract.transfer.getData(user1, OneMawCoin * 1);
    //

    const MSG1 = "send 1Maw and 1DENNIS to user1";
    const trSubmit = await walletInstance.requestTransferInMaw(user1, 1, 1, MSG1, {from:vault});
    ...
    assert.equal(afterBalance.toString(), OneMawCoinInMinunit.mul(BIG(1)).add(BIG(1)).toString());
  ...
  it("Add a signer", async () => {
    ...
    assertNotRevert("signer1",() => walletInstance.requestTransferInMaw(user2, 1, 0, "test addsigner-1", {from: user1}));
  ...
  it("Remove a signer", async () => {
    ...
    assertRevert("signer1",() => walletInstance.requestTransferInMaw(user1, 1, 0, "test addsigner-1", {from: user1}));
