...
/**
 * MawCoin contract learning test
 *   deploy in before, and no deploy between it()
 */
contract("MawCoin", (accounts) => {
  ...
  let NoOfTokensInMinunit, OneMawCoinInMinunit, NoOfTokensInMaw;
  let coin
  ...
  before(async () => {
    ...
    OneMawCoinInMinunit = BIG(await coin.getOneMawCoin());
    NoOfTokensInMaw = NoOfTokensInMinunit.div(OneMawCoinInMinunit)
  });
  ...
  it("coin initial values", async () => {
    ...
    assert.equal(await coin.symbol(), "MAW");
    assert.equal(await coin.name(), "MAWCoin");
    assert.equal(await coin.decimals(), 9);
    assert.equal((await coin.totalSupply()).toString(), BIG(NOOFTOKENS).mul(OneMawCoinInMinunit).toString());
  ...
  it("token distribution", async () => {
    ...
    await coin.transfer(admin, OneMawCoinInMinunit, {from: vault});
    assert.equal(await balanceOf(admin), OneMawCoinInMinunit.toString());

    // transfer 0.5 token from admin to owner
    let halfMaw = OneMawCoinInMinunit.div(BIG(2))
    await coin.transfer(owner, halfMaw, {from: admin});
    assert.equal(await balanceOf(owner), halfMaw.toString());
    assert.equal(await balanceOf(admin), halfMaw.toString());
    assert.equal(await balanceOf(vault), (NoOfTokensInMaw.sub(BIG(1)).mul(OneMawCoinInMinunit).toString()));
  });
  ...
  it("marginal transfers", async () => {
    ...
    try {
      await coin.transfer(user2, OneMawCoinInMinunit.mul(BIG(1)), {from: user1});
      assert.fail();
    }
    ...
    try
    {
      await coin.transfer(user2, OneMawCoinInMinunit.mul(BIG(0)), {from: user1});
      assert.equal(await balanceOf(user1), OneMawCoinInMinunit.mul(BIG(0)).toString());
      assert.equal(await balanceOf(user2), OneMawCoinInMinunit.mul(BIG(0)).toString());
    }
    catch(exception) { assert.fail(); }

    // transfer 0 token from vault(balance > 0) to user1
    try {
        await coin.transfer(user1, OneMawCoinInMinunit.mul(BIG(0)), {from: vault});
        assert.equal(await balanceOf(user1), OneMawCoinInMinunit.mul(BIG(0)).toString());
    } catch(exception) { assert.fail(); }

    // transfer whole balance to other
    await coin.transfer(user1, OneMawCoinInMinunit.mul(BIG(1)), {from: vault});
    assert.equal(await balanceOf(user1), OneMawCoinInMinunit.mul(BIG(1)).toString());
    await coin.transfer(user2, OneMawCoinInMinunit.mul(BIG(1)), {from: user1});
    assert.equal(await balanceOf(user1), OneMawCoinInMinunit.mul(BIG(0)).toString());
    assert.equal(await balanceOf(user2), OneMawCoinInMinunit.mul(BIG(1)).toString());
  });
  ...
  it("only admin can set reserve", async () => {
    await coin.setReserve(user1, OneMawCoinInMinunit, {from: admin});
  ...
  it("reserve and reserveOf", async () => {
    ...
    await coin.transfer(user1, OneMawCoinInMinunit.mul(BIG(10)), { from: vault });
    await coin.setReserve(user1, OneMawCoinInMinunit.mul(BIG(5)), { from: admin });
    assert.equal(await reserveOf(user1), OneMawCoinInMinunit.mul(BIG(5)).toString());
  });
  
  it("transfer after setting reservation", async () => {
    // user1 = 10 Maw
    await coin.transfer(user1, OneMawCoinInMinunit.mul(BIG(10)), { from: vault });
    // user1 = 10 Maw with 5 Maw reserve
    await coin.setReserve(user1, OneMawCoinInMinunit.mul(BIG(5)), { from: admin });
    // user1 = 9 Maw with 5 Maw reserve
    await coin.transfer(user2, OneMawCoinInMinunit.mul(BIG(1)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneMawCoinInMinunit.mul(BIG(9)).toString());

    // user1 = 6 Maw with 5 Maw reserve
    coin.transfer(user2, OneMawCoinInMinunit.mul(BIG(3)), {from: user1});        // keeping reserved amount
    assert.equal(await balanceOf(user1), OneMawCoinInMinunit.mul(BIG(6)));

    try {
      let tx = await coin.transfer(user2, OneMawCoinInMinunit.mul(BIG(2)), {from: user1}); // violating reserved amount
      assert.equal(await balanceOf(user1), OneMawCoinInMinunit.mul(BIG(6)));
      assert.fail();
    }
    catch(exception) {
